...
public class JVXClassElement extends CompositeElement implements Constants {
    ...
    public JVXClassElement(IElementType type) {
        super(type);
    }
    ...
    public TreeElement addInternal(TreeElement first, ASTNode last, ASTNode anchor, Boolean before) {
        ...
        for (ASTNode child = first; child != afterLast; child = next) {
            ...
            if (child.getElementType() == JVXElementType.METHOD && ((PsiMethod)SourceTreeToPsiMap.treeElementToPsi(child)).isConstructor()) {
        ...
        if (psiClass.isEnum()) {
            for (ASTNode child = first; child != afterLast; child = next) {
                next = child.getTreeNext();
                if ((child.getElementType() == JVXElementType.METHOD && ((PsiMethod)SourceTreeToPsiMap.treeElementToPsi(child)).isConstructor()) ||
                        child.getElementType() == JVXElementType.ENUM_CONSTANT) {
            ...
            for (ASTNode child = first; child != afterLast; child = next) {
                ...
                if (childElementType == JVXElementType.METHOD || childElementType == JVXElementType.FIELD) {
                    CompositeElement modifierList = (CompositeElement)((CompositeElement)child).findChildByRole(ChildRole.MODIFIER_LIST);
                    final TokenSet removeModifiersBitSet = level8OrHigher && childElementType == JVXElementType.METHOD
