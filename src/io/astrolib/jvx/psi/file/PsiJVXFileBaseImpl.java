...
public abstract class PsiJVXFileBaseImpl extends PsiFileImpl implements PsiJavaFile {
    ...
    protected PsiJVXFileBaseImpl(IElementType elementType, IElementType contentElementType, FileViewProvider viewProvider) {
        super(elementType, contentElementType, viewProvider);
        myResolveCache = CachedValuesManager.getManager(myManager.getProject()).createCachedValue(new PsiJVXFileBaseImpl.MyCacheBuilder(this), false);
    }
    ...
    public PsiClass[] getClasses() {
        // TODO(skeswa): investigate how the stubbed verison of this works.
        final StubElement<?> stub = getStub();
        if (stub != null) {
            return stub.getChildrenByType(JVXStubElementTypes.CLASS, PsiClass.ARRAY_FACTORY);
        }
    ...
    public PsiPackageStatement getPackageStatement() {
        ASTNode node = calcTreeElement().findChildByType(JVXElementType.PACKAGE_STATEMENT);
    ...
    public PsiImportList getImportList() {
        ...
        if (stub != null) {
            // TODO(skeswa): investigate how the stubbed verison of this works.
            PsiImportList[] nodes = stub.getChildrenByType(JVXStubElementTypes.IMPORT_LIST, PsiImportList.ARRAY_FACTORY);
            if (nodes.length != 1) {
        ...
        ASTNode node = calcTreeElement().findChildByType(JVXElementType.IMPORT_LIST);
    ...
    public boolean processDeclarations(@NotNull final PsiScopeProcessor processor,
        ...
        if (processor instanceof ClassResolverProcessor &&
            ...
            PsiJVXFileBaseImpl.MyResolveCacheProcessor cacheProcessor = new PsiJVXFileBaseImpl.MyResolveCacheProcessor(processor, state);
    ...
    private boolean processDeclarationsNoGuess(PsiScopeProcessor processor, @NotNull ResolveState state, PsiElement lastParent, PsiElement place) {
        ...
        if (importStaticStatements.length > 0) {
            final PsiJVXFileBaseImpl.StaticImportFilteringProcessor staticImportProcessor = new PsiJVXFileBaseImpl.StaticImportFilteringProcessor(processor);
    ...
    private static final NotNullLazyKey<LanguageLevel, PsiJVXFileBaseImpl> LANGUAGE_LEVEL_KEY = NotNullLazyKey.create("LANGUAGE_LEVEL", new NotNullFunction<PsiJVXFileBaseImpl, LanguageLevel>() {
        @Override
        @NotNull
        public LanguageLevel fun(PsiJVXFileBaseImpl file) {
    ...
    private static class MyCacheBuilder implements CachedValueProvider<MostlySingularMultiMap<String, SymbolCollectingProcessor.ResultWithContext>> {
        private final PsiJVXFileBaseImpl myFile;

        public MyCacheBuilder(PsiJVXFileBaseImpl file) {
            myFile = file;
        }
