...
import static io.astrolib.jvx.psi.parser.JVXParserUtil.braceMatchingBuilder;
import static io.astrolib.jvx.psi.parser.JVXParserUtil.expectOrError;
import static io.astrolib.jvx.psi.parser.JVXParserUtil.error;
import static io.astrolib.jvx.psi.parser.JVXParserUtil.exprType;
import static io.astrolib.jvx.psi.parser.JVXParserUtil.done;
import static io.astrolib.jvx.psi.parser.JVXParserUtil.emptyElement;
import static io.astrolib.jvx.psi.parser.JVXParserUtil.stoppingBuilder;
...
public class DeclarationParser {
  ...
  private static final TokenSet AFTER_END_DECLARATION_SET = TokenSet.create(
    JVXElementType.FIELD, JVXElementType.METHOD);
  ...
  private static final TokenSet RESOURCE_EXPRESSIONS = TokenSet.create(
    JVXElementType.REFERENCE_EXPRESSION, JVXElementType.THIS_EXPRESSION);
  ...
  public DeclarationParser(@NotNull final JVXParser javaParser) {
    myParser = javaParser;
  }
  ...
  public PsiBuilder.Marker parseClassFromKeyword(final PsiBuilder builder, final PsiBuilder.Marker declaration,
    ...
    refParser.parseReferenceList(builder, JavaTokenType.EXTENDS_KEYWORD, JVXElementType.EXTENDS_LIST, JavaTokenType.COMMA);
    refParser.parseReferenceList(builder, JavaTokenType.IMPLEMENTS_KEYWORD, JVXElementType.IMPLEMENTS_LIST, JavaTokenType.COMMA);
    ...
    done(declaration, JVXElementType.CLASS);
    return declaration;
  }
  ...
  public PsiBuilder.Marker parseEnumConstant(final PsiBuilder builder) {
    ...
    if (expect(builder, JavaTokenType.IDENTIFIER)) {
      if (builder.getTokenType() == JavaTokenType.LPARENTH) {
        ...
        emptyElement(builder, JVXElementType.EXPRESSION_LIST);
      }

      if (builder.getTokenType() == JavaTokenType.LBRACE) {
        final PsiBuilder.Marker constantInit = builder.mark();
        parseClassBodyWithBraces(builder, false, false);
        done(constantInit, JVXElementType.ENUM_CONSTANT_INITIALIZER);
      }

      done(constant, JVXElementType.ENUM_CONSTANT);
      return constant;
    }
    else {
      constant.rollbackTo();
  ...
  public PsiBuilder.Marker parse(final PsiBuilder builder, final Context context) {
    ...
    if (builder.getTokenType() == JavaTokenType.LBRACE) {
      ...
      done(declaration, JVXElementType.CLASS_INITIALIZER);
      return declaration;
    }
    ...
    if (TYPE_START.contains(builder.getTokenType())) {
      ...
      if (type == null) {
        ...
        if (typeParams == null) {
          emptyElement(builder, JVXElementType.TYPE_PARAMETER_LIST);
        }
    ...
    if (builder.getTokenType() == JavaTokenType.LPARENTH) {
      if (context == Context.CLASS || context == Context.ANNOTATION_INTERFACE) {  // method
        if (typeParams == null) {
          emptyElement(type, JVXElementType.TYPE_PARAMETER_LIST);
        }
  ...
  public Pair<PsiBuilder.Marker, Boolean> parseModifierList(final PsiBuilder builder, final TokenSet modifiers) {
    ...
    done(modList, JVXElementType.MODIFIER_LIST);
  ...
  private PsiBuilder.Marker parseMethodFromLeftParenth(PsiBuilder builder, PsiBuilder.Marker declaration, boolean anno, boolean constructor) {
    ...
    myParser.getReferenceParser().parseReferenceList(builder, JavaTokenType.THROWS_KEYWORD, JVXElementType.THROWS_LIST, JavaTokenType.COMMA);
    ...
    done(declaration, anno ? JVXElementType.ANNOTATION_METHOD : JVXElementType.METHOD);
    return declaration;
  }
  ...
  private PsiBuilder.Marker parseElementList(PsiBuilder builder, ListType type) {
    ...
    done(elementList, resources ? JVXElementType.RESOURCE_LIST : JVXElementType.PARAMETER_LIST);
    return elementList;
  }
  ...
  public PsiBuilder.Marker parseResource(PsiBuilder builder) {
    ...
    if (expr != null && RESOURCE_EXPRESSIONS.contains(exprType(expr)) && builder.getTokenType() != JavaTokenType.IDENTIFIER) {
      marker.done(JVXElementType.RESOURCE_EXPRESSION);
      return marker;
    }
  ...
  private PsiBuilder.Marker parseListElement(PsiBuilder builder, boolean typed, boolean ellipsis, boolean disjunctiveType, boolean resource) {
    ...
    if (typed) {
      ...
      if (typeInfo == null) {
        if (Boolean.TRUE.equals(modListInfo.second)) {
          ...
          emptyElement(builder, JVXElementType.TYPE);
        }
      }
    }
    ...
    if (typed) {
      ...
      if (tokenType == JavaTokenType.THIS_KEYWORD || tokenType == JavaTokenType.IDENTIFIER && builder.lookAhead(1) == JavaTokenType.DOT) {
        ...
        PsiBuilder.Marker expr = myParser.getExpressionParser().parse(builder);
        if (expr != null && exprType(expr) == JVXElementType.THIS_EXPRESSION) {
          mark.drop();
          done(param, JVXElementType.RECEIVER_PARAMETER);
          return param;
        }
    ...
    if (expect(builder, JavaTokenType.IDENTIFIER)) {
      if (!resource) {
        ...
        done(param, JVXElementType.PARAMETER);
        return param;
      }
    }
    else {
    ...
    done(param, JVXElementType.RESOURCE_VARIABLE);
    return param;
  }
  ...
  private PsiBuilder.Marker parseFieldOrLocalVariable(PsiBuilder builder, PsiBuilder.Marker declaration, int declarationStart, Context context) {
    ...
    if (context == Context.CLASS || context == Context.ANNOTATION_INTERFACE) {
      varType = JVXElementType.FIELD;
    }
    else if (context == Context.CODE_BLOCK) {
      varType = JVXElementType.LOCAL_VARIABLE;
    }
    else {
      declaration.drop();
  ...
  public PsiBuilder.Marker parseAnnotation(final PsiBuilder builder) {
    ...
    done(anno, JVXElementType.ANNOTATION);
    return anno;
  }
  ...
  private PsiBuilder.Marker parseAnnotationParameterList(final PsiBuilder builder) {
    ...
    if (!expect(builder, JavaTokenType.LPARENTH)) {
      done(list, JVXElementType.ANNOTATION_PARAMETER_LIST);
      return list;
    }
  ...
  private boolean parseAnnotationParameter(final PsiBuilder builder, final boolean mayBeSimple) {
    ...
    if (mayBeSimple) {
      ...
      if (builder.getTokenType() != JavaTokenType.EQ) {
        done(pair, JVXElementType.NAME_VALUE_PAIR);
        return false;
      }
  ...
  private PsiBuilder.Marker parseAnnotationArrayInitializer(final PsiBuilder builder) {
    ...
    if (expect(builder, JavaTokenType.RBRACE)) {
      done(annoArray, JVXElementType.ANNOTATION_ARRAY_INITIALIZER);
      return annoArray;
    }
