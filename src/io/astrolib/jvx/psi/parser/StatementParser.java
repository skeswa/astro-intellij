...
import static io.astrolib.jvx.psi.parser.JVXParserUtil.*;
...
public class StatementParser {
    ...
    private final JVXParser myParser;

    public StatementParser(@NotNull final JVXParser javaParser) {
        myParser = javaParser;
    }
    ...
    public PsiBuilder.Marker parseCodeBlock(final PsiBuilder builder, final boolean isStatement) {
        ...
        codeBlock.collapse(JVXElementType.CODE_BLOCK);
        if (greedyBlock) {
    ...
    public PsiBuilder.Marker parseCodeBlockDeep(final PsiBuilder builder, final boolean parseUntilEof) {
        ...
        done(codeBlock, JVXElementType.CODE_BLOCK);
        if (greedyBlock) {
            codeBlock.setCustomEdgeTokenBinders(null, WhitespacesBinders.GREEDY_RIGHT_BINDER);
        }
        return codeBlock;
    ...
    public PsiBuilder.Marker parseStatement(final PsiBuilder builder) {
        ...
        if (tokenType == JavaTokenType.IF_KEYWORD) {
            ...
            return parseTagStatement(builder);
            ...
            builder.advanceLexer();
            done(empty, JVXElementType.EMPTY_STATEMENT);
            return empty;
            ...
            if (suspectedLT == JavaTokenType.LT || suspectedLT == JavaTokenType.DOT && next == JavaTokenType.AT) {
                ...
                done(declStatement, JVXElementType.DECLARATION_STATEMENT);
        ...
        if (expr != null) {
            ...
            if (count > 1) {
                pos.drop();
                done(list, JVXElementType.EXPRESSION_LIST);
                semicolon(builder);
                done(statement, JVXElementType.EXPRESSION_LIST_STATEMENT);
                return statement;
            }
            if (exprType(expr) != JVXElementType.REFERENCE_EXPRESSION) {
                drop(list, pos);
                semicolon(builder);
                done(statement, JVXElementType.EXPRESSION_STATEMENT);
                return statement;
            }
            pos.rollbackTo();
        } else {
        ...
        if (decl != null) {
            ...
            done(statement, JVXElementType.DECLARATION_STATEMENT);
            return statement;
        }
        ...
        if (builder.getTokenType() == JavaTokenType.IDENTIFIER && builder.lookAhead(1) == JavaTokenType.COLON) {
            ...
            done(statement, JVXElementType.LABELED_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseIfStatement(final PsiBuilder builder) {
        ...
        if (!parseExpressionInParenth(builder)) {
            done(statement, JVXElementType.IF_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseWhileStatement(final PsiBuilder builder) {
        ...
        if (!parseExpressionInParenth(builder)) {
            done(statement, JVXElementType.WHILE_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseForStatement(final PsiBuilder builder) {
        ...
        if (!expect(builder, JavaTokenType.LPARENTH)) {
            ...
            done(statement, JVXElementType.FOR_STATEMENT);
            return statement;
        }
        ...
        if (param == null || exprType(param) != JVXElementType.PARAMETER || builder.getTokenType() != JavaTokenType.COLON) {
    ...
    private void parseExpressionOrExpressionList(final PsiBuilder builder) {
        ...
        if (builder.getTokenType() != JavaTokenType.COMMA) {
            ...
            done(expressionStatement, JVXElementType.EXPRESSION_STATEMENT);
        } else {
            ...
            done(expressionList, JVXElementType.EXPRESSION_LIST);
            done(expressionStatement, JVXElementType.EXPRESSION_LIST_STATEMENT);
        }
    }
    ...
    private PsiBuilder.Marker parseForEachFromColon(PsiBuilder builder, PsiBuilder.Marker statement) {
        ...
        done(statement, JVXElementType.FOREACH_STATEMENT);
        return statement;
    }
    ...
    private PsiBuilder.Marker parseDoWhileStatement(final PsiBuilder builder) {
        ...
        if (body == null) {
            error(builder, JavaErrorMessages.message("expected.statement"));
            done(statement, JVXElementType.DO_WHILE_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseSwitchStatement(final PsiBuilder builder) {
        ...
        if (!parseExpressionInParenth(builder)) {
            done(statement, JVXElementType.SWITCH_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseSwitchLabelStatement(final PsiBuilder builder) {
        ...
        done(statement, JVXElementType.SWITCH_LABEL_STATEMENT);
        return statement;
    }
    ...
    private static PsiBuilder.Marker parseBreakStatement(final PsiBuilder builder) {
        ...
        semicolon(builder);
        done(statement, JVXElementType.BREAK_STATEMENT);
        return statement;
    }
    ...
    private static PsiBuilder.Marker parseContinueStatement(final PsiBuilder builder) {
        ...
        expect(builder, JavaTokenType.IDENTIFIER);
        semicolon(builder);
        done(statement, JVXElementType.CONTINUE_STATEMENT);
        return statement;
    }
    ...
    private PsiBuilder.Marker parseReturnStatement(final PsiBuilder builder) {
        ...
        semicolon(builder);
        done(statement, JVXElementType.RETURN_STATEMENT);
        return statement;
    }
    ...
    private PsiBuilder.Marker parseThrowStatement(final PsiBuilder builder) {
        ...
        if (expr == null) {
            error(builder, JavaErrorMessages.message("expected.expression"));
            done(statement, JVXElementType.THROW_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseSynchronizedStatement(final PsiBuilder builder) {
        ...
        if (!parseExpressionInParenth(builder)) {
            done(statement, JVXElementType.SYNCHRONIZED_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseTryStatement(final PsiBuilder builder) {
        ...
        if (tryBlock == null) {
            error(builder, JavaErrorMessages.message("expected.lbrace"));
            done(statement, JVXElementType.TRY_STATEMENT);
            return statement;
        }
    ...
    public boolean parseCatchBlock(final PsiBuilder builder) {
        ...
        if (!expect(builder, JavaTokenType.LPARENTH)) {
            error(builder, JavaErrorMessages.message("expected.lparen"));
            done(section, JVXElementType.CATCH_SECTION);
            return false;
        }
    ...
    private PsiBuilder.Marker parseAssertStatement(final PsiBuilder builder) {
        ...
        if (expr == null) {
            ...
            done(statement, JVXElementType.ASSERT_STATEMENT);
            return statement;
        }
    ...
    private PsiBuilder.Marker parseBlockStatement(final PsiBuilder builder) {
        ...
        done(statement, JVXElementType.BLOCK_STATEMENT);
        return statement;
    }

    @NotNull
    private PsiBuilder.Marker parseTagStatement(final PsiBuilder builder) {
        final PsiBuilder.Marker statement = builder.mark();
        builder.advanceLexer();

        // Default to opening tag.
        IElementType tagElement = JVXElementType.OPENING_TAG_STATEMENT;

        if (expect(builder, JavaTokenType.DIV)) {
            // Indicates a closing tag.
            tagElement = JVXElementType.CLOSING_TAG_STATEMENT;
        }

        if (myParser.getReferenceParser().parseJavaCodeReference(builder, false, true, false, false) == null) {
            // Invalid tag type identifier.
            error(builder, JVXErrorMessage.INVALID_TAG_TYPE.getMessage());
            done(statement, tagElement);
            return statement;
        }

        // If this is a closing tag, it should end right here.
        if (tagElement == JVXElementType.CLOSING_TAG_STATEMENT) {
            if (expect(builder, JavaTokenType.GT)) {
                done(statement, tagElement);
                return statement;
            } else {
                error(builder, JVXErrorMessage.EXPECTED_GT.getMessage());
                done(statement, tagElement);
                return statement;
            }
        }

        while (!expect(builder, JavaTokenType.GT)) {
            if (expect(builder, JavaTokenType.DIV)) {
                // Means that this tag is self-closing.
                tagElement = JVXElementType.SELF_CLOSING_TAG_STATEMENT;

                // Also means that the next token must be a ">".
                if (expect(builder, JavaTokenType.GT)) {
                    done(statement, tagElement);
                    return statement;
                } else {
            ...
            if (myParser.getExpressionParser().parseTagAttributeExpression(builder) == null) {
                // Invalid attribute. Error is already reported.
                done(statement, tagElement);
                return statement;
            }
        }
