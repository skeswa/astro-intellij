...
public class ExpressionParser {
    ...
    public ExpressionParser(@NotNull final JVXParser javaParser) {
        myParser = javaParser;
    }
    ...
    private PsiBuilder.Marker parseAssignment(final PsiBuilder builder) {
        ...
        if (ASSIGNMENT_OPS.contains(tokenType) && tokenType != null) {
            ...
            assignment.done(JVXElementType.ASSIGNMENT_EXPRESSION);
            return assignment;
        }
    ...
    public PsiBuilder.Marker parseConditional(final PsiBuilder builder) {
        ...
        if (truePart == null) {
            error(builder, JavaErrorMessages.message("expected.expression"));
            ternary.done(JVXElementType.CONDITIONAL_EXPRESSION);
            return ternary;
        }
    ...
    private PsiBuilder.Marker parseBinary(final PsiBuilder builder, final ExprType type, final TokenSet ops) {
        ...
        while (true) {
            ...
            if (tokenType == null || !ops.contains(tokenType) || tokenType != currentExprTokenType || right == null) {
                ...
                result.done(operandCount > 2 ? JVXElementType.POLYADIC_EXPRESSION : JVXElementType.BINARY_EXPRESSION);
                if (right == null) break;
    ...
    private PsiBuilder.Marker parseRelational(final PsiBuilder builder) {
        ...
        while ((tokenType = getGtTokenType(builder)) != null) {
            ...
            if (RELATIONAL_OPS.contains(tokenType)) {
                toCreate = JVXElementType.BINARY_EXPRESSION;
                toParse = ExprType.SHIFT;
            } else if (tokenType == JavaTokenType.INSTANCEOF_KEYWORD) {
                toCreate = JVXElementType.INSTANCE_OF_EXPRESSION;
    ...
    private PsiBuilder.Marker parseUnary(final PsiBuilder builder) {
        ...
        if (PREFIX_OPS.contains(tokenType)) {
            ...
            unary.done(JVXElementType.PREFIX_EXPRESSION);
            return unary;
            ...
            typeCast.done(JVXElementType.TYPE_CAST_EXPRESSION);
            return typeCast;
        } else {
            return parsePostfix(builder);
        }
    ...
    private PsiBuilder.Marker parsePostfix(final PsiBuilder builder) {
        ...
        while (POSTFIX_OPS.contains(builder.getTokenType())) {
            ...
            builder.advanceLexer();
            postfix.done(JVXElementType.POSTFIX_EXPRESSION);
            operand = postfix;
        }
    ...
    private PsiBuilder.Marker parsePrimary(PsiBuilder builder, @Nullable BreakPoint breakPoint, int breakOffset) {
        ...
        while (true) {
            ...
            if (tokenType == JavaTokenType.DOT) {
                ...
                if (dotTokenType == JavaTokenType.CLASS_KEYWORD && exprType(expr) == JVXElementType.REFERENCE_EXPRESSION) {
                    ...
                    builder.mark().done(JVXElementType.REFERENCE_PARAMETER_LIST);
                    builder.advanceLexer();
                    refExpr.done(JVXElementType.REFERENCE_EXPRESSION);
                    expr = refExpr;
                } else if (THIS_OR_SUPER.contains(dotTokenType) && exprType(expr) == JVXElementType.REFERENCE_EXPRESSION) {
                    ...
                    startMarker = copy;
                    expr = ref.precede();
                    expr.done(dotTokenType == JavaTokenType.THIS_KEYWORD ? JVXElementType.THIS_EXPRESSION : JVXElementType.SUPER_EXPRESSION);
                } else {
                    PsiBuilder.Marker refExpr = expr.precede();
                ...
                callExpr.done(JVXElementType.METHOD_CALL_EXPRESSION);
                expr = callExpr;
                ...
                if (builder.getTokenType() == JavaTokenType.RBRACKET && exprType(expr) == JVXElementType.REFERENCE_EXPRESSION) {
                    ...
                    if (index == null) {
                        error(builder, JavaErrorMessages.message("expected.expression"));
                        arrayAccess.done(JVXElementType.ARRAY_ACCESS_EXPRESSION);
                        startMarker.drop();
                        return arrayAccess;
                    }
    ...
    private PsiBuilder.Marker parsePrimaryExpressionStart(final PsiBuilder builder) {
        ...
        if (LITERALS.contains(tokenType)) {
            ...
            builder.advanceLexer();
            literal.done(JVXElementType.LITERAL_EXPRESSION);
            return literal;
        }
        ...
        if (tokenType == JavaTokenType.LPARENTH) {
            ...
            parenth.done(JVXElementType.PARENTH_EXPRESSION);
            return parenth;
        }
        ...
        if (tokenType == JavaTokenType.IDENTIFIER) {
            ...
            if (annotation != null) {
                ...
                refParam.doneBefore(JVXElementType.REFERENCE_PARAMETER_LIST, annotation);
        ...
        if (THIS_OR_SUPER.contains(tokenType)) {
            ...
            expr.done(builder.getTokenType() == JavaTokenType.LPARENTH
                    ? JVXElementType.REFERENCE_EXPRESSION
                    : tokenType == JavaTokenType.THIS_KEYWORD
                    ? JVXElementType.THIS_EXPRESSION
                    : JVXElementType.SUPER_EXPRESSION);
            return expr;
        }
    ...
    private PsiBuilder.Marker parseArrayInitializer(final PsiBuilder builder) {
        ...
        arrayInit.done(JVXElementType.ARRAY_INITIALIZER_EXPRESSION);
        return arrayInit;
    }
    ...
    private PsiBuilder.Marker parseNew(PsiBuilder builder, @Nullable PsiBuilder.Marker start) {
        ...
        if (tokenType == JavaTokenType.IDENTIFIER) {
            ...
            if (refOrType == null) {
                error(builder, JavaErrorMessages.message("expected.identifier"));
                newExpr.done(JVXElementType.NEW_EXPRESSION);
                return newExpr;
            }
        ...
        if (refOrType != null && builder.getTokenType() == JavaTokenType.LPARENTH) {
            ...
            if (builder.getTokenType() == JavaTokenType.LBRACE) {
                ...
                classElement.done(JVXElementType.ANONYMOUS_CLASS);
            }
            newExpr.done(JVXElementType.NEW_EXPRESSION);
            return newExpr;
        }
    ...
    private static PsiBuilder.Marker parseClassObjectAccess(PsiBuilder builder, PsiBuilder.Marker expr, boolean optionalClassKeyword) {
        ...
        expr.done(JVXElementType.CLASS_OBJECT_ACCESS_EXPRESSION);
        return expr;
    }
    ...
    private PsiBuilder.Marker parseMethodReference(final PsiBuilder builder, final PsiBuilder.Marker start) {
        ...
        start.done(JVXElementType.METHOD_REF_EXPRESSION);
        return start;
    }
    ...
    private PsiBuilder.Marker parseLambdaExpression(final PsiBuilder builder, final boolean typed, @Nullable final PsiBuilder.Marker typeList) {
        ...
        start.done(JVXElementType.LAMBDA_EXPRESSION);
        return start;
    }
    ...
    public PsiBuilder.Marker parseArgumentList(final PsiBuilder builder) {
        ...
        list.done(JVXElementType.EXPRESSION_LIST);
        if (!closed) {
    ...
    @Nullable
    PsiBuilder.Marker parseTagAttributeExpression(PsiBuilder builder) {
        PsiBuilder.Marker expr = builder.mark();

        if (!expect(builder, JavaTokenType.IDENTIFIER)) {
            error(builder, JavaErrorMessages.message("expected.identifier"));
            done(expr, JVXElementType.TAG_ATTRIBUTE_EXPRESSION);
            return null;
        }

        if (!expect(builder, JavaTokenType.EQ)) {
            // If there is no equal sign, then this is a stand-alone attribute.
            done(expr, JVXElementType.TAG_ATTRIBUTE_EXPRESSION);
            return expr;
        }

        // Look for either a string or a {expression}.
        if (expect(builder, JavaTokenType.STRING_LITERAL)) {
            // Found a string, we're done.
            done(expr, JVXElementType.TAG_ATTRIBUTE_EXPRESSION);
            return expr;
        } else if (expect(builder, JavaTokenType.LBRACE)) {
            // Read the expression that must be right here.
            final PsiBuilder.Marker innerExpr = parse(builder);
            if (innerExpr != null) {
                // Found the expr. Finish this off.
                done(expr, JVXElementType.TAG_ATTRIBUTE_EXPRESSION);
                return expr;
            }
        }

        error(builder, JVXErrorMessage.EXPECTED_STR_OR_EXPR.getMessage());
        done(expr, JVXElementType.TAG_ATTRIBUTE_EXPRESSION);
        return null;
    }

    private static void emptyExpression(final PsiBuilder builder) {
        emptyElement(builder, JVXElementType.EMPTY_EXPRESSION);
    }
