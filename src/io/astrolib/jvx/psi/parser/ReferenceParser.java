...
public class ReferenceParser {
  ...
  public ReferenceParser(@NotNull final JVXParser javaParser) {
    myParser = javaParser;
  }
      ...
      if (operator != null && builder.getTokenType() == operator) {
        ...
        while (builder.getTokenType() == operator) {
          ...
          if (tokenType != JavaTokenType.IDENTIFIER && tokenType != JavaTokenType.AT) {
            JVXParserUtil.error(builder, JavaErrorMessages.message("expected.identifier"));
          }
          parseTypeInfo(builder, flags, false);
        }

        typeInfo.marker.done(JVXElementType.TYPE);
      }
    }
  ...
  private TypeInfo parseTypeInfo(PsiBuilder builder, int flags, boolean badWildcard) {
    ...
    if (expect(builder, ElementType.PRIMITIVE_TYPE_BIT_SET)) {
      ...
      JVXParserUtil.emptyElement(builder, JVXElementType.DIAMOND_TYPE);
      type.done(JVXElementType.TYPE);
      typeInfo.marker = type;
      return typeInfo;
    }
    else {
      ...
      if (anno != null && isSet(flags, INCOMPLETE_ANNO)) {
        JVXParserUtil.error(builder, JavaErrorMessages.message("expected.type"));
  ...
  private PsiBuilder.Marker parseJavaCodeReference(PsiBuilder builder, boolean eatLastDot, boolean parameterList, boolean isImport,
    ...
    if (parameterList) {
      ...
      JVXParserUtil.emptyElement(builder, JVXElementType.REFERENCE_PARAMETER_LIST);
    }

    boolean hasIdentifier;
    while (builder.getTokenType() == JavaTokenType.DOT) {
      refElement.done(JVXElementType.JAVA_CODE_REFERENCE);
      ...
      if (!hasIdentifier) {
        typeInfo.hasErrors = true;
        if (isImport) {
          JVXParserUtil.error(builder, JavaErrorMessages.message("import.statement.identifier.or.asterisk.expected."));
          refElement.drop();
          return prevElement;
        }
        else {
    ...
    refElement.done(isStaticImport ? JVXElementType.IMPORT_STATIC_REFERENCE : JVXElementType.JAVA_CODE_REFERENCE);
    return refElement;
  }
  ...
  public boolean parseReferenceParameterList(final PsiBuilder builder, final boolean wildcard, final boolean diamonds) {
    ...
    if (!expect(builder, JavaTokenType.LT)) {
      list.done(JVXElementType.REFERENCE_PARAMETER_LIST);
      return false;
    }
    ...
    while (true) {
      ...
      else if (!JVXParserUtil.expectOrError(builder, JavaTokenType.COMMA, "expected.gt.or.comma")) {
        isOk = false;
        break;
      }
  ...
  public PsiBuilder.Marker parseTypeParameters(final PsiBuilder builder) {
    final PsiBuilder.Marker list = builder.mark();
    if (!expect(builder, JavaTokenType.LT)) {
      list.done(JVXElementType.TYPE_PARAMETER_LIST);
      return list;
    }
    ...
    while (true) {
      ...
      if (param == null) {
        JVXParserUtil.error(builder, JavaErrorMessages.message("expected.type.parameter"));
      }
    ...
    if (!expect(builder, JavaTokenType.GT)) {
      ...
      if (builder.getTokenType() == JavaTokenType.IDENTIFIER) {
        if (builder.lookAhead(1) == JavaTokenType.GT) {
          ...
          JVXParserUtil.error(builder, JavaErrorMessages.message("expected.gt"));
        }
      }
      else {
        JVXParserUtil.error(builder, JavaErrorMessages.message("expected.gt"));
  ...
  public PsiBuilder.Marker parseTypeParameter(final PsiBuilder builder) {
    ...
    parseReferenceList(builder, JavaTokenType.EXTENDS_KEYWORD, JVXElementType.EXTENDS_BOUND_LIST, JavaTokenType.AND);

    if (!wild) {
      param.done(JVXElementType.TYPE_PARAMETER);
    }
    else {
  ...
  private static boolean isKeywordAny(PsiBuilder builder) {
    return JVXParserUtil.getLanguageLevel(builder).isAtLeast(LanguageLevel.JDK_X) && "any".equals(builder.getTokenText());
  }

  private static void dummy(PsiBuilder builder) {
    PsiBuilder.Marker mark = builder.mark();
    builder.advanceLexer();
    mark.done(JVXElementType.DUMMY_ELEMENT);
  }
}
