...
public class JavadocParser {
  ...
  public static void parseJavadocReference(@NotNull final PsiBuilder builder) {
    JVXParser.INSTANCE.getReferenceParser().parseJavaCodeReference(builder, true, true, false, false);
    swallowTokens(builder);
  }

  public static void parseJavadocType(@NotNull final PsiBuilder builder) {
    JVXParser.INSTANCE.getReferenceParser().parseType(builder, ReferenceParser.EAT_LAST_DOT | ReferenceParser.ELLIPSIS | ReferenceParser.WILDCARD);
    swallowTokens(builder);
  }
  ...
  private static void parseDataItem(@NotNull final PsiBuilder builder, @Nullable final String tagName, final boolean isInline) {
    ...
    if (tokenType == JavaDocTokenType.DOC_INLINE_TAG_START) {
      ...
      if (SEE_TAG.equals(tagName) && !isInline ||
        ...
        if (JVXParserUtil.getLanguageLevel(builder).isAtLeast(LanguageLevel.JDK_1_4) && LINK_PLAIN_TAG.equals(tagName) && isInline) {
          parseSeeTagValue(builder, false);
        }
          ...
          if (JVXParserUtil.getLanguageLevel(builder).isAtLeast(LanguageLevel.JDK_1_5) && VALUE_TAG.equals(tagName) && isInline) {
