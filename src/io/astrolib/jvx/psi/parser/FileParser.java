...
public class FileParser {
  ...
  public FileParser(@NotNull final JVXParser javaParser) {
    myParser = javaParser;
  }
  ...
  public void parseFile(@NotNull final PsiBuilder builder,
    ...
    while (!builder.eof()) {
      ...
      if (declaration != null) {
        ...
        if (firstDeclarationOk == null) {
          firstDeclarationOk = JVXParserUtil.exprType(declaration) != JVXElementType.MODIFIER_LIST;
    ...
    if (impListInfo.second && firstDeclarationOk == Boolean.TRUE) {
      impListInfo.first.setCustomEdgeTokenBinders(JVXParserUtil.PRECEDING_COMMENT_BINDER, null);  // pass comments behind fake import list
      firstDeclaration.setCustomEdgeTokenBinders(JVXParserUtil.SPECIAL_PRECEDING_COMMENT_BINDER, null);
    }
  }
  ...
  public PsiBuilder.Marker parsePackageStatement(final PsiBuilder builder) {
    ...
    if (!expect(builder, JavaTokenType.PACKAGE_KEYWORD)) {
      ...
      JVXParserUtil.done(modList, JVXElementType.MODIFIER_LIST);
      if (!expect(builder, JavaTokenType.PACKAGE_KEYWORD)) {
    ...
    JVXParserUtil.semicolon(builder);

    JVXParserUtil.done(statement, JVXElementType.PACKAGE_STATEMENT);
    return statement;
  }
  ...
  public Pair<PsiBuilder.Marker, Boolean> parseImportList(final PsiBuilder builder, final TokenSet stoppers) {
    ...
    JVXParserUtil.done(list, JVXElementType.IMPORT_LIST);
  ...
  private PsiBuilder.Marker parseImportStatement(final PsiBuilder builder) {
    ...
    final IElementType type = isStatic ? JVXElementType.IMPORT_STATIC_STATEMENT : JVXElementType.IMPORT_STATEMENT;
    ...
    JVXParserUtil.done(statement, type);
    return statement;
  }
}
